...
def test_return_empty_if_df_empty():
    with pytest.warns(UserWarning) as record:
        empty_df = pd.DataFrame(data=[], columns=['c1', 'c2', 'c3'])
        created_df = PandasModelCreator(empty_df, 'TestModel').create()
...
def test_dynamically_convert_from_pandas_to_kafka_model(test_data, test_df):
    records = PandasModelCreator(
        test_df, 'TestModel', base=KafkaModel).create()
...
def test_fields_names_and_titles_are_the_same(test_df):
    records = PandasModelCreator(test_df, "TestModel").create()
...
def test_all_defaults_from_field_name(test_df):
    ...
    records = PandasModelCreator(
        test_df, "TestModel", fields_defaults=expected, base=KafkaModel).create()
...
def test_some_defaults_from_field_name(test_df):
    ...
    records = PandasModelCreator(
        test_df, "TestModel", fields_defaults=local_defaults, base=KafkaModel).create()
...
def test_all_defaults_from_field_type(test_df):
    ...
    records = PandasModelCreator(
        test_df, "TestModel", types_defaults=expected, base=KafkaModel).create()
...
def test_some_defaults_from_field_type(test_df):
    ...
    records = PandasModelCreator(
        test_df, "TestModel", types_defaults=local_defaults, base=KafkaModel).create()
...
def test_optional_fields_specified_by_param(test_df_with_nones):
    records = PandasModelCreator(test_df_with_nones, "TestModel", optional_fields=[
                                 'name'], base=KafkaModel).create()
...
def test_supported_types(value, _type):
    ...
    records = PandasModelCreator(df, "TestModel", base=KafkaModel).create()
...
def test_supported_optional_types(value, _type):
    df = pd.DataFrame({'column_1': [value]})
    records = PandasModelCreator(df, "TestModel", optional_fields=[
                                 'column_1'], base=KafkaModel).create()
    expected = {'column_1': _type}
